...
export const sendCdResponse = (
  ...
  res.send(`<div>
<script>
window.parent.postMessage({"vendor": "rdn", "type": "expand"}, '*');
</script>
<a target="_top" href="${click}">
<img src="${img}" style="width:100%;"></img>
</a>
</div>`);
};

export const sendCdVideoResponse = (
  res: express.Response,
  img: string,
  click: string
): void => {
...
// deprecated
export const sendCdA2AResponse = (res: express.Response, img: string): void => {
  res.setHeader('Content-Type', 'text/html');
  res.send(`<div>
<script>
window.parent.postMessage({"vendor": "rdn", "type": "expand"}, '*');
function clickAd(event) {
console.log("in clickAd() in creative");
window.parent.postMessage({"vendor": "rdn", "type": "open_popup", "url": "https://dev-s-ad.rmp.rakuten.co.jp/a2a?demand_id=ichiba"}, '*');
return false;
}
</script>
<a href="#" onclick="return clickAd(event);">
<img src="${img}" style="width:100%;"></img>
</a>
</div>`);
};
